...
# create bien first
b, _ = BienAsegurado.objects.get_or_create(descripcion='Bien sin test', defaults={'estado':1,'tipo_bien':1,'valor':500,'zona':z})
usuario, _ = Usuario.objects.get_or_create(email='cliente3@example.com', defaults={'nombre':'Cli3','apellido':'T3','telefono':''})
pol, created = Poliza.objects.get_or_create(numero_poliza='POL-TEST-3', defaults={'titular':usuario, 'tipo_poliza':'Automóvil', 'fecha_emision':date.today(), 'fecha_vencimiento':date.today(), 'prima':200.0, 'cobertura':'Cobertura X', 'estado':'activa', 'aseguradora':'UTPL', 'fecha_inicio':date.today(), 'fecha_fin':date.today(), 'bien':b})
if created:
